...
def run_test(alphaA, alphaB, filename, test_name):
    ...
    I_fast_mod : volt
    dVT_dyn/dt = (VT_base + I_fast_mod - VT_dyn)/tau_VT : volt
    ...
    Delta_VT = 0.2*mV  # очень слабая модуляция
    ...
    def fast_to_slow_modulation():
        ...
        Sigma_slow.I_fast_mod = Delta_VT * mean_fast

    #S_fast_to_slow.connect(p=0.2)
    # Инициализация
    Sigma_slow.v = EL
    Sigma_slow.w = 0*pA
    ...
    # Запуск сети
    net = Network(Sigma_slow, S_rec,Sigma_fast, M_spike, M_v, M_w, M_fast_spike, M_fast_v, M_VT, fast_to_slow_modulation)
    net.run(sim_time)
