...
print("Эмбриональный режим Σ_slow — три теста по каналам A/B (OU-шума) — Шаг 3")
...
# Сила обратной связи Σ_fast → w Σ_slow
Delta_w = 0.05*pA
...
def run_test(alphaA, alphaB, filename, test_name):
    ...
    # --------------------------
    # Мягкая обратная связь Σ_fast → w Σ_slow
    # --------------------------
    @network_operation(dt=10*ms)
    def fast_to_slow_modulation_w():
        mean_s_fast = np.mean(Sigma_fast.s_fast)
        # добавляем Δw
        Sigma_slow.w += Delta_w * mean_s_fast
        # ограничиваем через Brian2 встроенные условия
        Sigma_slow.w = where(Sigma_slow.w > 20*pA, 20*pA, Sigma_slow.w)
        Sigma_slow.w = where(Sigma_slow.w < 0*pA, 0*pA, Sigma_slow.w)
    ...
    # Запуск сети
    # --------------------------
    net = Network(Sigma_slow, S_rec, Sigma_fast,
                  M_spike, M_v, M_w, M_fast_spike, M_fast_v, M_VT,
                  fast_to_slow_modulation_w)
    net.run(sim_time)
